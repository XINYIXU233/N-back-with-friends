...
for n in range(1,4):
    ...
    instruction.draw()
    win.flip()
    event.waitKeys('space') #wait for space to continue
...
# 3. export data
data = [['ID', 'Anser_List','Letter_Display','Block', 'Key_Pressed', 'Response', 'Reaction_Time']]
...
filename = f'sub-{ID:02d}' # data filename for each sub.
